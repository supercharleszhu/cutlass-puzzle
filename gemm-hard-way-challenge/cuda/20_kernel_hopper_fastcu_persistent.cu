...
// Day 20 blank focus: make the fast.cu-like Hopper tile persistent.
#include "challenge_todo.cuh"


// FILL-IN BLANKS FOR THIS FILE
//   [ ] Blank A: compare persistent scheduling with one-CTA-per-output-tile.
//   [ ] Blank B: connect CUTLASS PersistentScheduler to fast.cu's fixed-SM loop.
//   [ ] Blank C: reason about load balance at small versus large matrix sizes.
//   [ ] Blank D: profile launch occupancy and SM residency.
//   [ ] Blank E: decide when Stream-K from Day 16 is preferable.
...
using Day20FastCuPersistentConfig = day20_standalone::HopperFastCuConfig<
    128, 256, 64,
    1, 1,
    day20_standalone::SchedulerKind::Persistent>;
...
void sgemm_cutlass_hopper_fastcu_persistent_bf16(
    ...
    torch::Tensor &output_matrix)
{
    day20_standalone::torch_wrapper<Day20FastCuPersistentConfig>(
        matrix_a, matrix_b, output_matrix, "Day 20 Hopper persistent GEMM");
}
